...
const PostCard = ({post}) => {
    ...
    const hashtags=post.content.replace(/(#\w+)/g, '<span class="text-indigo-600">$1</span>')
  ...
  return (
    <div className='bg-linear-to-b from-white to-slate-300 rounded-xl shadow p-4 space-y-4 w-full max-w-2xl'>
      ...
      {post.content && <div className='text-gray-800 text-sm whitespace-pre-line' dangerouslySetInnerHTML={{__html: hashtags}}/>}
      ...
      <div className='flex items-center gap-4  text-gray-700 text-sm pt-2 border-t border-gray-300'>
        <div className='flex items-center gap-1'>
          <Heart className={`w-7 h-7 cursor-pointer ${likes.includes(currentUser._id) && 'text-red-500 fill-red-500 '}`} onClick={()=>handleLike()}/>
